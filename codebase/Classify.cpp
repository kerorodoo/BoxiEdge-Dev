...
int Classify::GtiClassify(float *InputData, int Count)
{
    ...
    std::cout << "\nClassify::GtiClassify clear predict!" << "\n";
    if (m_Predications.size())
    {
        m_Predications.clear();
    }

    std::cout << "\nClassify::GtiClassify fill input buffer!" << "\n";
    ...
    std::cout << "\nClassify::GtiClassify gtiAccelerateFC!" << "\n";
    fcOut = gtiAccelerateFC(&fcOutLen);

    num = std::min<int>(m_Labels.size(), Count);
    maxN = gtiArgmax(fcOut, fcOutLen, num);
    for (int i = 0; i < num; ++i)
    {
        index = maxN[i];
        lable_id = std::to_string(index) + " : " + m_Labels[index];
...
{
    m_PredText = std::to_string(m_Predications[Index].second) + string(" - ") + m_Predications[Index].first;
